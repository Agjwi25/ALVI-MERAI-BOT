...
api.changeNickname(`${(!global.config.BOTNAME) ? "Made by O M A R" : global.config.BOTNAME}`, handleReply.pending[singleIndex - 1].threadID, api.getCurrentUserID());
            api.sendMessage("", event.threadID, () => api.sendMessage({body:` البوت كورومي في خدمتك `, attachment: fs.createReadStream(__dirname + "/rankup/trolled.mp3")} ,handleReply.pending[singleIndex - 1].threadID));
            count+=1;
...
module.exports.run = async function({ api, event, args, permission, handleReply }) {
    ...
    case "-u":
    case "المستخدمين": {
    const permission = ["100019463832721"];
